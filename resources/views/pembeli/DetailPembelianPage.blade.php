    ...
    <div class="border border-gray-300 max-w-4xl mx-auto mt-8 bg-white p-6 rounded-lg">
        <div class="flex flex-col grid-cols-2 md:flex-row gap-x-10">
            <div class="flex-shrink-0 md:pr-16">
                <p class="text-xl md:text-2xl font-libre-franklin font-bold tracking-tight text-gray-900 py-2">
                    {{ $order->product->type->name }}</p>
                <img src="{{ $order->product->img_link }}" alt="{{ $order->product->type->name }}"
                ...
                <p class="text-base md:text-lg font-libre-franklin italic tracking-tight text-gray-900">
            ...
            <form action="{{ route('order.cancel', ['order' => $order->id]) }}" method="POST">
                ...
                <div class="flex justify-end gap-4">
                    <button type="button" onclick="closePopup()"
                        class="border border-black inline-flex px-4 py-2 bg-white rounded-lg gap-x-2 hover:bg-gray-200">
