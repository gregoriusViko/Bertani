<x-layout>
    ...
    <div class="mt-4 px-7 mx-auto max-w-screen-lg" x-data="{ imageUrl: '/img/noimage.png' }">
        <form enctype="multipart/form-data" method="POST" action="{{ route('product.update', $product->slug) }}"
            class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-7 mx-auto max-w-full">
            @csrf
            @method('PUT')
            {{-- kiri gambar --}}
            <div>
                    ...
                    <div class="flex justify-between">
                        ...
                        <div class="">
                            <button type="submit" name="delete_photo" value="1"
                                class="px-1 py-1 bg-red-600 text-white rounded-lg hover:bg-red-400 text-sm">Hapus
                                Foto</button>
                        </div>
                    </div>

                    <input id="foto" name="foto" type="file" accept="image/*"
                        class="block mt-1 w-full border p-2 rounded-md shadow-md">
