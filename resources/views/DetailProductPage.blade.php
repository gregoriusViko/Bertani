                    ...
                    <div class="rounded-md grid grid-flow-col">
                        <div class="w 1/2">
                            ...
                            <div class="flex items-center">
                                ...
                                <h4 class="font-inter text-sm md:text-base font-normal ml-1 hover:underline">{{ $product->farmer->home_address }}</h4>
                            </div>
        ...
        <form action="{{ route('order.store') }}" method="POST" class="row-span-4 col-span-2 flex items-center">
            ...
            <div class="text-start w-full">
                <div class="border border-black rounded-md w-full">
                    ...
                    <x-text-area id="deskripsi" class="block mt-1 w-full rounded-lg border border-black" type="text" name="deskripsi" readonly>{{ $product->description }}</x-text-area>
                </div>
                <div class="mt-4">
                    ...
                    <div class="border rounded-md grid grid-flow-col">
                        <div class="w 1/2">
                            <div class="font-inter font-normal text-sm md:text-md lg:text-lg">
                                ...
                                <div class="mt-2 grid grid-cols-2 grid-auto-columns:auto">
                                    <input type="number" id="stokInput" name="quantity" min="1" class="w-3/4 md:w-1/2 ml-2 border border-black rounded-md" placeholder="" data-price="{{ $product->price }}" data-max-stock="{{ $product->stock_kg }}" oninput="calculateSubtotal()" required />
                        ...
                        <div class="w 1/2">
                            <div class="font-inter font-normal text-sm md:text-md lg:text-lg mr-2">
                                ...
                                <button type="button" id="peran" onclick="toggleDropdown()" class="mt-2 px-4 py-2 w-full shadow-lg border bg-white text-gray-400 text-sm font-inter font-normal border-gray-300 focus:border-green-600 outline-none rounded-lg hover:bg-gray-50 justify-start">
                                ...
                                <ul id="dropdownMenu" class="absolute hidden shadow-[0_8px_19px_-7px_rgba(6,81,237,0.2)] bg-white py-2 z-[99] divide-y max-h-96 overflow-auto rounded-lg mt-1">
                                    <li onclick="selectOption('Cash')" class="py-3 px-5 hover:bg-green-400 text-gray-800 text-sm font-inter font-normal cursor-pointer">Cash</li>
                                    <li onclick="selectOption('Transfer')" class="py-3 px-5 hover:bg-green-400 text-gray-800 text-sm font-inter font-normal cursor-pointer">Transfer</li>
                                </ul>
